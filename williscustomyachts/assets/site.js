...
(() => {
  ...
  /* ---------- text Ron: phone taps straight to SMS; desktop shows the QR + email panel ---------- */
  const isIOS = /iP(hone|ad|od)/.test(navigator.userAgent) || (/Mac/.test(navigator.userAgent) && "ontouchend" in document);
  const coarse = matchMedia("(pointer: coarse)").matches;
  const RON_PHONE = "+15614104253";
  const RON_MSG = "Hey Ron, following up on the electric bill review for Willis Custom Yachts (Jack James Drive). What are next steps?";
  document.querySelectorAll(".js-sms-toggle").forEach((btn) => {
    btn.addEventListener("click", () => {
      if (coarse) {
        location.href = "sms:" + RON_PHONE + (isIOS ? "&" : "?") + "body=" + encodeURIComponent(RON_MSG);
        return;
      }
  ...
  /* "Book the fit call" routes to Ron's text/email flow for now (no CPL scheduling link yet).
     ensureCal above is intentionally left unwired until a CPL Cal.com link exists. */
  void ensureCal;
  document.querySelectorAll("[data-book]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const scope = btn.closest("section, .hero__in, .close__in");
      let panel = scope && scope.querySelector(".sms");
      let toggle = scope && scope.querySelector(".js-sms-toggle");
      if (!panel) {
        // header / global button: no local panel, so reveal the hero contact and scroll to it
        const hero = document.querySelector(".hero");
        panel = hero && hero.querySelector(".sms");
        toggle = hero && hero.querySelector(".js-sms-toggle");
  ...
  if (gateEl && gateCalc) {
    ...
    if (localStorage.getItem("ep-unlocked")) {
      ...
      form.addEventListener("submit", (e) => {
        ...
        fetch("https://formsubmit.co/ajax/ron@cpl.vision", {
          method: "POST",
          headers: { "Content-Type": "application/json", "Accept": "application/json" },
          body: JSON.stringify({ email, _subject: "Estimator unlock (new lead)", page: "cpl-hotel-landing", _template: "table" }),
        }).then(finish, finish);
      });
    }
  }
  ...
  if (ledger) {
    ...
    /* demand- and motor-relevant first: a Willis reader should meet power
       conditioning, motor, and demand-adjacent results before the rest of the record */
    const rank = (r) => {
      const s = r[1] + " " + r[2] + " " + r[3];
      if (/power factor|power condition|demand|motor|pump|compressor/i.test(s)) return 0;
      if (/hvac|rooftop|chiller|office|building|manufacturing|logistics/i.test(s)) return 1;
      return 2;
    };
